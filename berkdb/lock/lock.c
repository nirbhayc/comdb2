...
upgrade:
	...
	case GRANT:
		newl->status = DB_LSTAT_HELD;
                /* NC: Lock (newl) is inserted to the holders' list here */
	...
	case SECOND:
		...
		switch (action) {
                /* NC: Lock (newl) is inserted to the waiters' list here */
		case HEAD:
		...
		case SECOND:
			...
			break;
		case TAIL:
                        if (gbl_enable_cats == 0) {
                            SH_TAILQ_INSERT_TAIL(&sh_obj->waiters, newl, links);
                        } else {
                            /* NC: Check for the total number of waiters for all
